// floor8.js (cine-style reservar y confirmar + modo edición)
(() => {
  ...
  // 1) Asientos Piso 8
  // ==========================
  // Arrancamos vacío y lo llenás con el modo edición (E + ShiftClick).
  // Formato:
  // { id: "8-01", x: 33.0, y: 62.0 }
  const SEATS_8 = [{ id: "8-01", x: 7.2, y: 32.8 },
...
{ id: "8-66", x: 78.1, y: 34.0 }];
  ...
  let occupied = new Set(); // ocupados definitivos (DB)
  ...
  // ==========================
  // MODO EDICIÓN (E) + ShiftClick para capturar coordenadas
  // ==========================
  const picked = [];
  let nextIdx = 1;
  let hud = null;
  ...
  // Shift + click: agrega punto + lo imprime en consola
  floorMap.addEventListener("click", (ev) => {
    if (!editMode) return;
    if (!ev.shiftKey) return;

    // evitamos capturar clicks en botones asiento
